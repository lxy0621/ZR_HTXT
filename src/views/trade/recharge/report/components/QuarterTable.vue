<template>
  <div>
    ...
    <el-table :header-cell-style="{background:'#eef1f6',color:'#606266'}" :data="tableData" size="mini" border ref="multipleTable"
      ...
      <el-table-column prop="dateNo" label="季度" align="center" min-width="130">
        <template slot-scope="scope">
          {{scope.row.dateNo}}
        </template>
      </el-table-column>
      <el-table-column prop="dateNo" label="对应日期" align="center" min-width="170">
        <template slot-scope="scope">
  ...
  export default {
    ...
    methods: {
      ...
      getDataList(params) {
        ...
        this.rechargeOrderService.pageQuarterList(this.dataForm).then(response => {
