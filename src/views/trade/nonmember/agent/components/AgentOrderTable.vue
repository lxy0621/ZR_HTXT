<template>
  <div>
    ...
    >
      ...
      <el-table-column prop="handle" label="操作" fixed align="center" width="150">
        <template slot-scope="scope">
          <el-link type="primary" :underline="false" @click="handleDetail(scope.row)" size="small">详情&ensp;</el-link>
          <el-link type="primary" :underline="false" @click="handleClose(scope.row)" size="small">关闭&ensp;</el-link>
          <el-link type="primary" :underline="false" @click="handleFinish(scope.row)" size="small">完成&ensp;</el-link>
      ...
      <el-table-column prop="qrcode" label="二维码号" align="center" min-width="140"></el-table-column>
    </el-table>
    <el-pagination
    ...
    <el-drawer :visible.sync="detailDrawer" :with-header="false" style="text-align: left;">
      ...
      <div style="margin-top: 20px;">
        ...
        <el-row :gutter="20">
          <el-col :span="8">
            <div class="grid-content-right">二维码号</div>
          </el-col>
          <el-col :span="16">
            <div class="grid-content-left">{{ detailData.qrcode }}</div>
          </el-col>
        </el-row>
        <el-row :gutter="20">
          <el-col :span="8">
            <div class="grid-content-right">更新人</div>
          </el-col>
          <el-col :span="16">
            <div class="grid-content-left">{{ detailData.updateName }}</div>
          </el-col>
        </el-row>
        <el-row :gutter="20">
          <el-col :span="8">
            <div class="grid-content-right">更新时间</div>
          </el-col>
          <el-col :span="16">
            <div class="grid-content-left">{{ detailData.updatedAt }}</div>
          </el-col>
        </el-row>
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="16">
            <div class="grid-content-left">代记账</div>
          </el-col>
        </el-row>
        <el-row :gutter="20">
...
<script>
  import {AgentOrderService} from '@/service/trade/agent/AgentOrderService.js';
  import * as formatter from "@/utils/formatter";

  export default {
    name: "AgentOrderTable",
    data() {
      return {
        selectRows: [],
        tableData: [],
        ...
        tableLoading: false,
        agentOrderService: null,
        detailData: {},
        detailDrawer: false,
      }
    },
    mounted() {
      this.agentOrderService = new AgentOrderService();
    ...
    methods: {
      ...
      getList(queryParams) {
        ...
        this.agentOrderService.pagingAgentOrders(this.queryParams)
          .then(response => {
      ...
      handleClose(row) {
        this.$confirm('确定关闭该订单?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning',
        }).then(() => {
          this.agentOrderService.closeAgentOrder(row.id)
            .then(response => {
      ...
      handleFinish(row) {
        this.$confirm('确定完成该订单?', '提示', {
          ...
          this.agentOrderService.finishAgentOrder(row.id)
            .then(response => {
              if (response.success === true) {
                this.$message({
                  message: response.message,
      ...
      handleDelete(row) {
        this.$confirm('确定删除该订单?', '提示', {
          ...
          this.agentOrderService.deleteAgentOrder(row.id)
            .then(response => {
              if (response.success === true) {
                this.$message({
                  message: response.message,
