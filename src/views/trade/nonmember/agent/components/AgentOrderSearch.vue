<template>
  <search-container @search="handleDataSearch" @reset="handleAllReset" can-fold>
    <search-container-item>
      <el-input v-model="queryParams.unionQuery" placeholder="订单号/收款二维码" size="small"/>
    </search-container-item>
    <search-container-item>
    ...
    <search-container-item>
      <el-select v-model="queryParams.isBoundQRCode" placeholder="是否绑定二维码" size="small" style="width:100%;" multiple clearable>
        <el-option v-for="item in boundQRCodes" :key="item.value" :label="item.key" :value="item.value"></el-option>
      </el-select>
    </search-container-item>
    <search-container-item>
      <el-row :gutter="5">
