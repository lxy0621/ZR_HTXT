<template>
	<div>
      ...
      <el-table-column prop="totalCount" label="总订单数" align="center" min-width="100" sortable></el-table-column>
			<el-table-column prop="dateNo" label="季度" align="center" min-width="120">
				<template slot-scope="scope">
					{{ scope.row.dateNo }}
				</template>
			</el-table-column>
      <el-table-column prop="dateNo" label="对应日期" align="center" min-width="220">
        <template slot-scope="scope">
...
export default {
    ...
    formatNumber(num) {
      return formatter.formatNumber(num);
		...
		getList(queryParams) {
			...
			this.service.pagingOrderQuarterReport(queryParams).then(response => {
    ...
    exportOrderQuarterReport(queryParams) {
			this.service.exportOrderQuarterReport(queryParams).then(response => {
