          ...
          <el-form :model="couponForm" ref="couponForm" label-width="110px" size="medium">
            ...
            <el-row :gutter="10">
              <el-form-item label="优惠券批次名" class="required" prop="name">
                <el-input v-model="couponForm.batchName" placeholder="请输入优惠券批次名" style="width: 100%;"></el-input>
              </el-form-item>
            </el-row>
            ...
            <el-form-item label="有效期" class="required" prop="validateRuleType">
              <el-radio-group v-model="couponForm.validateRuleType" style="width: 100%" @change="onChangeValidateRuleType">
                <el-radio label="1" value="1" style="display: block; margin-bottom: 10px;">领取当日起&ensp;
                  <el-input-number v-model="couponForm.fromCurrentDays" :precision="0" :min="1" :max="90" :controls="false" style="width: 419px;"
    ...
    mounted() {
      ...
      if ({} != this.$route.params) {
        ...
        this.couponForm = {
          ...
          validateRuleType: '' + param.validateRuleType,
          skuIds: param.skuIdsByArray,
        ...
        // 领取当日起
        if (param.validateRuleType == 1) {
          this.couponForm.fromCurrentDays = param.validateDays;
        }
        if (param.validateRuleType == 2) {
          this.couponForm.fromMorrowDays = param.validateDays;
        }
        if (param.validateRuleType == 3) {
          this.dateRange = [param.fromDate, param.toDate];
        }
      }
    },
    methods: {
      ...
      submit() {
        ...
        this.couponAdminService.updateCouponBatch(this.couponForm)
          .then(response => {
