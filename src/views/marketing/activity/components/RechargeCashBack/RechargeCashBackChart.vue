...
<script>
import RechargeCashBackBtnGroups from "./RechargeCashBackBtnGroups";
import { ActivityAdminService } from "@/service/marketing/activity/ActivityAdminService";
export default {
  name: "RechargeCashBackChart",
  components: {
  ...
  data() {
    return {
      ...
      chartData: {
        columns: ["日期范围", "充值次数", "充值用户数"],
        rows: []
      },
      chartData2: {
        columns: ["日期范围", "活动充值金额", "活动返现金额"],
        rows: []
      },
      chartSettings: {
        yAxisType: ["NUM"],
      ...
      this.activityAdminService
        ...
        .then(response => {
          ...
          var newChartData = ChartData.map(item => ({
            日期范围: item.xdisplay,
            充值次数: item.totalOrder,
            充值用户数: item.totalUser
          }));
          var newChartData2 = ChartData.map(item => ({
            日期范围: item.xdisplay,
            活动充值金额: item.totalInMoney,
            活动返现金额: item.totalOutMoney
          }));
