...
export default {
  ...
  mounted() {
    this.vipDay = new VipDayAdminService();
    this.findVipDayGroups(this.queryParams);
  },
  methods: {
    toggleContainer(index) {
      this.active = index;
    },
    findVipDayGroups(queryParams) {
      this.queryParams = queryParams;
      this.vipDay.findMemFestivalConfigGroups(queryParams).then(response => {
        this.total = response.data.total; //查询结果
        this.searchBtnData[0].children[0].childNum = response.data.stateCard.TO_BEGIN;
        this.searchBtnData[0].children[1].childNum = response.data.stateCard.ONGOING;
        this.searchBtnData[0].children[2].childNum = response.data.stateCard.CLOSED;
        this.searchBtnData[0].children[3].childNum = response.data.stateCard.EXPIRED;
      });
    },
    handleSearchTotal() {
    ...
    handleSearch(obj, pindex, cindex) {
      ...
      this.queryParams.stateCard = obj.state;
      this.$emit("search", this.queryParams);
    }
  }
};
