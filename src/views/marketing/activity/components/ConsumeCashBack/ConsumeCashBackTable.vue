<template>
  <div>
    ...
    >
      ...
      <el-table-column prop="giftMoney" label="优惠金额（元）" align="center" width="120">
        <template slot-scope="scope">
          <span style="font-weight: bold; color: red;">{{formatNumber(scope.row.giftMoney)}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="actualPayMoney" label="实际消费（元）" align="center" width="110">
        <template slot-scope="scope">
          <span style="font-weight: bold; color: red;">{{formatNumber(scope.row.actualPayMoney)}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="rpName" label="返现红包" align="center" width="110"></el-table-column>
      <el-table-column prop="redMoney" label="返现金额（元）" align="center" width="110">
        <template slot-scope="scope">
          <span style="font-weight: bold; color: red;">{{formatNumber(scope.row.redMoney)}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="outTradeNo" label="订单号" align="center" width="260"></el-table-column>
...
export default {
  ...
  mounted() {
    ...
    if (this.$route.query !== undefined) {
      ...
      this.loadConsumeCashBackData(this.queryParams);
    }
  },
  methods: {
    initSelectData() {
    ...
    loadConsumeCashBackData(params) {
