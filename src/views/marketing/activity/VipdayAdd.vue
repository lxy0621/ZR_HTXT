    ...
    <el-card class="card">
      <div class="content">
        <el-form ref="form" label-width="130px" :model="form" size="medium">
          ...
          <el-form-item prop="intervals" label="会员日规则：">
            <template>
          ...
          <el-form-item prop="activityId" label="关联活动：">
...
export default {
  ...
  methods: {
    ...
    //提交表单
    submit(form) {
      form.fromDate = this.form.dateRange[0] + " 00:00:00";
      form.toDate = this.form.dateRange[1] + " 23:59:59";
      if (this.form.name == undefined) {
      ...
      } else if (this.form.activityId == undefined) {
        this.$message({
          message: "会员日关联活动不能为空",
          type: "warning",
          duration: 1600
        ...
        let queryParams = {
          ...
          name: form.name,
          fromDate: form.dateRange[0] + " 00:00:00",
          toDate: form.dateRange[1] + " 23:59:59",
          loopType: parseInt(form.loopType),
        ...
        this.$refs["form"].validate(valid => {
          if (valid) {
            this.vipDayAdminService
              .addMemFestivalConfig(queryParams)
              .then(response => {
                if (response.success === true) {
                  ...
                  closeCurrentTabAndToTab("会员日管理");
                  this.$emit("findVipDayGroups", this.queryParams);
                } else {
                  this.$message({
                    message: response.message,
                    type: "error"
